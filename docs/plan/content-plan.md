...
Status 2026-09-24: all 16 units are **own drafts** on the book's unit topics (greetings and
nationality, family, housing, daily routine, food, prayer, study, work, shopping, weather,
people and places, hobbies, travel, Hajj and Umra, health, holidays — topics inferred from the
publisher's page videos, to be confirmed against the printed book), marked `"status": "entwurf"` until the teacher
reviews them; the app shows the draft note in the unit header. The earlier demo dialogue of unit
2 was removed because it followed the book's dialogue too closely. Pronouns and particles carry
an empty root (no root cards). Verb tables are generated by pattern and so far only for sound form-I verbs; weak,
hamzated, doubled and derived-form verbs (e.g. سافَرَ، قالَ، أَكَلَ) need hand-checked tables.

Example sentences come from Tatoeba (`tools/content/tatoeba-examples.mjs` finds candidates:
