...
- `apps/api/src/storage/`: `ObjectStorage` with `S3ObjectStorage` (`@aws-sdk/client-s3`,
  path style). Presigned URLs are signed for the internal endpoint and rewritten to
  `/media/<bucket>/<key>`; Caddy strips `/media` and proxies with that Host header. Keys are
  generated by the server and validated before use.
- Parts are 32 MB (below proxy body limits). Play URLs live 6 hours so a long lesson can be
  sought without a new URL; upload URLs 15 minutes.
- Configuration: `SUFFA_S3_ENDPOINT`, `SUFFA_S3_ACCESS_KEY_ID`, `SUFFA_S3_SECRET_ACCESS_KEY`
  (+ optional region and bucket names). Without them recordings are off.
- CI runs the storage and recording tests against moto's S3 server.
